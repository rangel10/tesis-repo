% OptForce para Saccharomyces

% ID / carpeta
runID = 'Result OptForce S';
...
% MustL set: Reacciones de primer orden que deben disminuir
[mustLSet, pos_mustL] = findMustL(model_test, minFluxesW, maxFluxesW, 'constrOpt', constrOpt, 'runID', runID, 'outputFolder', 'OutputsFindMustL', 'outputFilename', 'MustL', ...
    'printExcel', true, 'printText', true, 'printReport', true, 'keepInputs', true, 'verbose', false);

% MustU set: Reacciones de primer orden que deben aumentar
[mustUSet, pos_mustU] = findMustU(model_test, minFluxesW, maxFluxesW, 'constrOpt', constrOpt, 'runID', runID, 'outputFolder', 'OutputsFindMustU', 'outputFilename', 'MustU', ...
    'printExcel', true, 'printText', true, 'printReport', true, 'keepInputs', true, 'verbose', false);
...
% MustUL set: Reacciones de segundo orden que aumentan y disminuyen
% [mustUL, pos_mustUL, mustUL_linear, pos_mustUL_linear] = findMustUL(model_test, minFluxesW, maxFluxesW, 'excludedRxns', excludedRxns, 'constrOpt', constrOpt, 'runID', runID, 'outputFolder', 'OutputsFindMustUL', 'outputFilename', 'MustUL', ...
%     'printExcel', true, 'printText', true, 'printReport', true, 'keepInputs', true, 'verbose', false);
...
% [optForceSets, posOptForceSets, typeRegOptForceSets, flux_optForceSets] = optForce(model_test, targetRxn, biomassRxn, mustU, mustL, minFluxesW, maxFluxesW, minFluxesM, maxFluxesM, 'k', k, 'nSets', nSets,...
%     'constrOpt', constrOpt, 'runID', runID, 'outputFolder', 'OutputsOptForce', 'outputFilename', 'OptForce', ...
%     'printExcel', true, 'printText', true, 'printReport', true, 'keepInputs', true, 'verbose', false);
